...
const ChatBox: React.FC<ChatBoxProps> = ({ selectedUser, role, sendMessage }) => {
    ...
    const renderMessages = () => {
        return <ul>
            {messages!
                .filter(message => message.senderId === selectedUser?.id || message.receiverId === selectedUser?.id)
                .map(message => {
                    let classes: string;
                    ...
                    return <li key={message.id} className={classes}>
                        <p>{message.content}</p>
                    </li>
                })}
        </ul>
    ...
    return (
        <>
            {!selectedUser && (<div className="non-selected">Select User to display chat</div>)}
            {selectedUser && (
                <div className="chat-box">
                    ...
                    <div className="chat-box-display">
                        ...
                        <form className="chat-box-input" onSubmit={handledSubmit}>
                            <input type="text" placeholder="Type your message..." onChange={handledChange} value={content} />
