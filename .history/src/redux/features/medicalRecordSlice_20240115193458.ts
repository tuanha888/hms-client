...
import { createTreatmentPlan } from "../actions/treatmentPlan-actions";
...
export const medicalRecordSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        builder.addCase(createTreatmentPlan.fulfilled, (state, action) => {
            state.medicalRecords = state.medicalRecords.filter(med => {
                if (med.id !== action.payload.medicalRecordId) return med
                else return {
                    ...med,
