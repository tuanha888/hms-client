...
const UserList: React.FC<UserListProp> = ({ role, selectUser }) => {
    ...
    const renderUserList: () => ReactNode = () => {
        if (userList.length === 0) {
            let content: string;
            if (role === "PATIENT") content = "No contacted doctors";
            else content = "No contacted patients";
        ...
        return <ul id="user-list">
            {userList
                ...
                .map(user => {
                    return <li key={user.id} id={user.id} className="chat-list-user-item" onClick={() => handleClick(user)}>
                        {role === "PATIENT"
    ...
    return (
        <div className="chat-list">
            <div className="chat-list-search">
                <span><FaSearch /></span>
                <input type="text" name="search" id="search" placeholder="Search..." value={searchTerm} onChange={handleChange} />
            </div>
            <div className="chat-list-user">
                {isFetched && (renderUserList())}
            </div>
        </div>
    )
}
